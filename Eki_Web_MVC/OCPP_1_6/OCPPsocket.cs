...
namespace OCPP_1_6
{
    public class OCPPsocket : FleckSocket<OCPP_Msg>
    {
        ...
        {
            //Log.d($"ocpp socket on close path->{socket.ConnectionInfo.Path}");
            //Log.d($"ocpp socket onClose instance->{socket.GetHashCode()}");
            removeCP(socket);
        }

        protected override void OnSocketMessage(IWebSocketConnection socket, OCPP_Msg msg)
        {
            //Log.d($"ocpp socket on message->{msg.toJsonString()} msgType->{msg.getMsgType()}");

            //Log.d($"ocpp socket onMessage instance->{socket.GetHashCode()}");
        ...
        protected override void OnSocketOpen(IWebSocketConnection socket)
        {
            ...
            //Log.d($"ocpp socket onOpen  instance->{socket.GetHashCode()}");

            //目前只有序號
            //var serial = socket.ConnectionInfo.Path.Substring(1);
            //Log.d($"ocpp onOpen serial->{serial}");

            //var oldCp = (from c in cpList
            //             where c.serial == serial
            //             select c).toSafeList();

            //Log.d($"old cp size->{oldCp.Count}");
        ...
        public void addCP(ChargePoint newCp)
        {
            ...
            //Log.d($"newCp socket->{newCp?.socket?.GetHashCode()} oldCp socket->{cp?.socket?.GetHashCode()}");
            //Log.d($"newCp socket serial->{newCp?.serial} oldCp socket->{cp?.serial}");
            if (cp == null)
            {
                cpList.Add(newCp);
            }
        ...
        //            Log.d($"ocpp socket Boot client->{client.toJsonString()}");
        ...
        //            Log.d($"ocpp socket Boot response data->{bootResult.toJsonString()}");
        ...
        //            Log.d($"ocpp socket StatusNotify response data->{statusResult.toJsonString()}");
        ...
        //            Log.d($"ocpp socket Heartbeat response data->{beatResult.toJsonString()}");
